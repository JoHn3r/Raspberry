...
if username != '':
    if browser.login(settings.value["url_address"], {'username': username, 'pass': password, 'action': 'login'},
...
def extract_torrents(data):
    ...
    if data is not None:
        ...
        for link in links:
            ...
            magnet = columns[2].a["href"]  # magnet
            size = columns[3].text.strip()  # size
            seeds = columns[6].text  # seeds
            peers = columns[7].text  # peers
...
def search_general(info):
    ...
    url_search = "%s/all/type-all/by-seed/desc/page1/%s" % (settings.value["url_address"], query)
